...
#' @param population The population data, as generated by `generate_population_data`
#'
#' @family variables
#' @export
create_variables <- function(parameters_list, population_data) {
  ...
  household_variable <- individual::CategoricalVariable$new(
    categories = as.character(1L:length(population_data$setting_sizes$household)),
    initial_values = as.character(population_data$initial_household_settings)
  )

  school_variable <- individual::CategoricalVariable$new(
    categories = as.character(0L:length(population_data$setting_sizes$school)),
    initial_values = as.character(population_data$initial_school_settings)
  )

  workplace_variable <- individual::CategoricalVariable$new(
    categories = as.character(0L:length(population_data$setting_sizes$workplace)),
    initial_values = as.character(population_data$initial_workplace_settings)
  )
  ...
  specific_leisure_variable <- individual::CategoricalVariable$new(
    ...
    initial_values = rep(as.character(0L), parameters_list$human_population)
  )
...
#' other locations. When enabled, it will also assign UVC status for each
#' location.
#'
#' @param parameters_list A list of model parameters as generated by `get_parameters`
#' @export
generate_population_data <- function(parameters_list) {
  ...
  # length(population_data$leisure_setting_sizes) because the indices in actual_assigned_leisure_settings
  ...
  num_leisure <- length(leisure_setting_sizes)
  ...
  household_specific_riskiness <- generate_setting_specific_riskinesses(
    parameters_list = parameters_list,
    setting = "household",
    number_of_locations = num_households
  )
  workplace_specific_riskiness <- generate_setting_specific_riskinesses(
    parameters_list = parameters_list,
    setting = "workplace",
    number_of_locations = num_workplaces
  )
  school_specific_riskiness <- generate_setting_specific_riskinesses(
    parameters_list = parameters_list,
    setting = "school",
    number_of_locations = num_schools
  )
  leisure_specific_riskiness <- generate_setting_specific_riskinesses(
    parameters_list = parameters_list,
    setting = "leisure",
    number_of_locations = num_leisure
  )

  setting_sizes = strict_list(
  ...
  data <- strict_list(
    ...
    leisure_indices = assigned_leisure_locations,
  ...
  if (
    ...
    data$uvc_switches <- generate_far_uvc_switches(parameters_list, setting_sizes)
  }
