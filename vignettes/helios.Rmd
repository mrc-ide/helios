...
```{r}
parameters_list <- get_parameters(list(human_population = 1000))
```
...
age_df <- purrr::map_df(age_classes, function(x) {
  ...
  data.frame("individual" = indices, "age_class" = x)
})
...
Each week, individuals attend lesiure venues:
...
We use a fixed number of timesteps to simulate over:
