...
Fig XA shows the previous household size distribution used (based on UK-wide data); Fig XB shows the household size distribution now being used (which is based on the data described above).
Fig Y shows the same but for the age distribution used.

**[[ Adam - can you insert some figures here please!]]**
...
Fig ZA shows the previous school size distribution used (based on UK-wide data); Fig ZB shows the household size distribution now being used (which is based on the data described above).
...
b <- ggplot(overall_df2, aes(x = as.numeric(time), y = ratio, col = setting_type)) +
  ...
  geom_vline(data = y_intercepts, aes(xintercept = time_spent)) +
  lims(y = c(0, 10)) +
  theme_bw() +
  scale_x_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12)) +
