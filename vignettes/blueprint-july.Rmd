...
These differences between the two pathogen archetypes arises primarily because of their different $R_0$ values.In Figure \@ref(fig:placeholder-2) we show the analytical relationship between $R_0$ and prevalence of infection, derived for a SEIRS compartmental model (see [here](https://www.nature.com/articles/s41592-020-0856-2)) and [here](https://shiny.bcgsc.ca/posepi2/)) that shares a similar representation of a disease's natural history to `helios`.
We note that the results presented in \@ref(fig:placeholder-2) are not results from running `helios` and are displayed here solely for the purpose of illustrating a generally-held non-linearity between transmissibility ($R_{0}$) and disease burden (in this case infection prevalence).
...
This means that when $R_0$ is high (as for the "SARS-CoV-2-Like" archetype), small reductions in $R_0$ (e.g. as a result of the installation of far UVC) will have only a slight impact on infection prevalence.
By contrast, when the baseline $R_0$ is lower (as for the "Influenza-Like" archetype"), the infection prevalence (and hence burden of disease) will decrease more (in relative terms) for the same reduction in $R_0$.
