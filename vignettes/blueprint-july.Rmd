...
(ref:wells-riley-1): Estimating the variation in riskiness between Locations belonging to the same Setting Type.
A) For each Setting Type, the probability of infection in the least and most risky settings (i.e. with the lowest and highest ventilation rates) over time.
B) The ratio of infection risk in the most vs least risky Locations over time for each Setting Type.
Dashed lines indicate the time assumed spent in each Setting Type for purposes of modelling.
Note ratios are the same for Leisure and Workplace and so only Workplace is plotted.
...
# Assessing the impact of far UVC on the burden of an endemic respiratory virus {#assessing}

Using the modelling framework described above, we carried analyses to evaluate the potential impact of far UVC deployment on transmission and disease burden of a hypothetical endemic respiratory virus (i.e. one which is consistently present in a population and maintained at a particular baseline prevalence level).
...
-   **far UVC Coverage:** Either 10%, 25% or 50% - in all cases, far UVC was assumed to be installed in schools, workplaces and leisure locations only (i.e. not households), and within each Setting Type, a random set of locations .

-   **far UVC Efficacy:** Either 40%, 60% and 80%, with the assumption that far UVC efficacy was identical across all Settings and Locations where it had been installed.
...
(ref:placeholder-1): The impact of varying far UVC coverage and efficacy on the annual incidence of a respiratory virus, for each of the pathogen archetypes considered.
Values presented by the bars represent the mean averaged over the 10 stochastic model simulations, with the range of % reduction in incidence across those 10 simulations shown by the error bars.


## Discussion

The observed differences in predicted far UVC impact between the two pathogen archetypes arises primarily because of their different $R_0$ values.
...
We note that the results presented in \@ref(fig:analytical-relationship) are not results from running `helios` (instead they are the analytical solution of a significantly more tractable mathematical model that is simpler but similar to `helios`) and are displayed here solely for the purpose of illustrating a generally-held non-linearity between transmissibility ($R_{0}$) and disease burden (in this case infection prevalence).
...
At higher values of $R_{0}$ however, infection prevalecne saturates and increasing values of $R_0$ have a diminishing impact on increasing it.
...
(ref:analytical-relationship): The relationship between $R_0$ and the prevalence of infection at endemic equilibrium for each of the two pathogen archetypes considered here, derived mathematically for a simpler model that is similar to `helios`.
Vertical dashed line indicates the value of $R_0$ used for each archetype in the analyses carried out.
