...
parameters_list_usa <- get_parameters(list(human_population = human_population, seed = 1))
...
plot_household_usa <- table(household_sizes_usa) |>
  data.frame() |>
  ggplot(aes(x = household_sizes_usa, y = Freq)) +
  geom_col() +
...
plot_household_uk <- table(household_sizes_uk) |>
  data.frame() |>
  ggplot(aes(x = household_sizes_uk, y = Freq)) +
  geom_col() +
  labs(x = "Number of individuals in the household", y = "Count")
...
plot_age_usa <- data.frame(age_classes_usa, age_class_counts_usa) |>
  ...
  ggplot(aes(x = age_classes_usa, y = age_class_counts_usa)) +
  geom_col() +
...
plot_age_uk <- data.frame(age_classes_uk, age_class_counts_uk) |>
  ...
  ggplot(aes(x = age_classes_uk, y = age_class_counts_uk)) +
  geom_col() +
  labs(x = "Age group", y = "Number of individuals") +
  coord_flip()
...
As school sizes were provided in ranges (e.g. "500 to 599") we used the midpoint (e.g. 550) for the purposes of constructing the distribution of schools sizes.
This data was available only at the national level, which we use here.

Figure \@ref(fig:school-size)A shows the previous school size distribution used (based on UK data); Figure \@ref(fig:school-size)B shows the household size distribution now being used (based on the USA data described above).
