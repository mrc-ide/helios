...
Susceptible individuals may become exposed after spending time in the same location as infectious individuals.
The probability of infection during one time-step of the model of size $\delta > 0$ is
$$
1 - \exp(- \lambda \delta).
$$
The term $\lambda$ is the force of infection, and given by a sum of location specific force of infections, together with a community level force of infection which represents general mixing among all people.
The force of infection for each location is given by the proportion of individuals in that location who are infectious, multiplied by a constant term representing the proportion of total transmission which occurs in each setting.
...
By altering the distribution of this final delay, the time between recovery and susceptibility, the framework has the capacity to simulate both epidemic and endemic pathogens (Figure \@ref(fig:epidemic-endemic)).
...
Coverage may be set to either at random or targetted (Figure \@ref(fig:uvc)).
...
(ref:uvc) Illustration of the targetted and random strategies for the workplace location type.
...
data.frame(workplace_sizes) |>
  ...
  ggplot(aes(x = rank, y = log10(workplace_sizes) + 1, fill = chosen)) +
  geom_col() +
  facet_wrap(~ type) +
  labs(x = "", y = "Base 10 logarithm of the workplace size plus one", fill = "Far UVC installed?")
