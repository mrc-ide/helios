...
```{r epidemic-peak-size, fig.cap="(ref:epidemic-peak-size)", fig.fullwidth = TRUE, fig.height=5}  # Assuming 10% coverage and 60% efficacy, far UVC installation led to a A% (range B%-C%) reduction in the epidemic peak size the "Influenza-Like" archetype, and X% (range Y%-Z%) for the "SARS-CoV-2-Like" archetype. At 10% coverage and 80% assumed efficacy, these are A% (range B%-C%) and X% (range Y%-Z%) respectively.  epidemic_summary_outputs |>   filter(coverage < 0.8, efficacy > 0) |>   ggplot(aes(x = 100 * coverage, y = mean_peak / 1000, colour = coverage_type)) +   geom_line(linewidth = 1) +   scale_colour_manual(values = blueprint_colours, labels = c("Random", "Targeted")) +   scale_fill_manual(values = blueprint_colours, labels = c("Random", "Targeted")) +   geom_ribbon(aes(ymin = min_peak / 1000, ymax = max_peak / 1000, fill = coverage_type),               colour = NA, alpha = 0.3, show.legend = FALSE) +   scale_x_continuous(expand = c(0, 0)) +   theme_bw(base_size = 12) +   labs(x = "Far UVC Coverage (%)",        y = "Mean Peak Epidemic Size \n (per 1,000 individuals)",        colour = "Coverage\nType") +   facet_grid(archetype ~ efficacy,              scales = "free_y",              labeller = as_labeller(c(`influenza` = "Influenza",                                       `sars_cov_2` = "SARS-CoV-2",                                       `0.4` = "40% Efficacy",                                       `0.6` = "60% Efficacy",                                       `0.8` = "80% Efficacy"))) +   theme(axis.text = element_text(colour = "black"),         axis.title = element_text(size = 12),         legend.text  = element_text(size = 11)) +   lims(y = c(0, NA))}
```
...
```{r epidemic-peak-timing, fig.cap="(ref:epidemic-peak-timing)", fig.fullwidth = TRUE, fig.height=5}  # For 10% coverage and 60% efficacy, far UVC installation led to the epidmic peak occurring A days (range B-C days) later for the "Influenza-Like" archetype and X days (range Y days-Z days) for the "SARS-CoV-2-Like" archetype, compared to a situation in which far UVC has not been installed. At 10% coverage and 80% assumed efficacy, these are A days (range B - C days) and X days (range Y days - Z days) respectively.  epidemic_summary_outputs |>   filter(coverage < 0.8, efficacy > 0) |>   filter(!(coverage > 0.5 & efficacy == 0.8 & coverage_type == "targeted_riskiness" & archetype == "influenza")) |>   ggplot(aes(x = 100 * coverage, y = mean_peak_timing, colour = coverage_type)) +   geom_line(linewidth = 1) +   scale_colour_manual(values = blueprint_colours, labels = c("Random", "Targeted")) +   scale_fill_manual(values = blueprint_colours, labels = c("Random", "Targeted")) +   geom_ribbon(aes(ymin = min_peak_timing, ymax = max_peak_timing, fill = coverage_type),               colour = NA, alpha = 0.3, show.legend = FALSE) +   scale_x_continuous(expand = c(0, 0)) +   theme_bw(base_size = 12) +   labs(x = "Far UVC Coverage (%)",        y = "Time to Peak Infections (days)",        colour = "Coverage\nType") +   facet_grid(archetype ~ efficacy,              scales = "free_y",              labeller = as_labeller(c(`influenza` = "Influenza",                                       `sars_cov_2` = "SARS-CoV-2",                                       `0.4` = "40% Efficacy",                                       `0.6` = "60% Efficacy",                                       `0.8` = "80% Efficacy"))) +   theme(axis.text = element_text(colour = "black"),         axis.title = element_text(size = 12),         legend.text  = element_text(size = 11)) +   lims(y = c(0, NA))}
```
...
```{r epidemic-final-size, fig.cap="(ref:epidemic-final-size)", fig.fullwidth = TRUE, fig.height=5}  # For 10% coverage and 60% efficacy, far UVC installation led to a reduction in the total % of the population becoming infected by A% (range B%-C%) for the "Influenza-Like" archetype. Without far UVC, X% (Y%-Z%) of individuals were infected, compared to X% (Y%-Z%) when with far UVC installed at 10% coverage and 60% efficacy. For the "SARS-CoV-2-Like" archetype, Without far UVC, X% (Y%-Z%) of individuals were infected, compared to X% (Y%-Z%) when with far UVC installed at 10% coverage and 60% efficacy. This corresponds to a reduction in the total % of the population becoming infected by A% (range B%-C%). SOMETHING COMPARING TARGETED AND UNTARGETED APPROACHES  epidemic_summary_outputs |>   filter(coverage < 0.8, efficacy > 0) |>   ggplot(aes(x = 100 * coverage, y = 100 * mean_final_size / 1e5, colour = coverage_type)) +   geom_line(linewidth = 1.2) +   scale_colour_manual(values = blueprint_colours, labels = c("Random", "Targeted")) +   scale_fill_manual(values = blueprint_colours, labels = c("Random", "Targeted")) +   geom_ribbon(aes(ymin = lq_final_size / 1000, ymax = uq_final_size / 1000, fill = coverage_type),               colour = NA, alpha = 0.3, show.legend = FALSE) +   scale_x_continuous(expand = c(0, 0)) +   theme_bw(base_size = 12) +   labs(x = "Far UVC Coverage (%)",        y = "Mean Final Epidemic Size \n (% of population)",        colour = "Coverage\nType") +   facet_grid(archetype ~ efficacy,              scales = "free_y",              labeller = as_labeller(c(`influenza` = "Influenza",                                       `sars_cov_2` = "SARS-CoV-2",                                       `0.4` = "40% Efficacy",                                       `0.6` = "60% Efficacy",                                       `0.8` = "80% Efficacy"))) +   theme(axis.text = element_text(colour = "black"),         axis.title = element_text(size = 12)) +   lims(y = c(0, NA))}
```
